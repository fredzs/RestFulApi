...
class EmailService(object):
    ...
    def make_msg(self, subject, sender_name, receiver_name, html_content):
        msg = MIMEText(html_content, 'html', 'utf-8')
        msg['From'] = self.format_addr('%s<%s>' % (sender_name, self._from_addr))
        msg['To'] = self.format_addr('%s<%s>' % (receiver_name, self._to_addr[0]))
    ...
    def send_range_email(self, date_begin, date_end):
        try:
            self.read_config()
            title_line, data, total_line = self.make_statistics(date_begin, date_end)
            html_content = self.data_to_html(title_line, data, total_line)
            msg = self.make_msg('望京支行机构金融业务部', '望京支行对公营销团队', '每日对公业绩统计_' + date_begin,  html_content)
    ...
    def make_statistics(self, date_begin, date_end):
        ...
        # logger.info("123123")
        return title_line, data, total_line

    def data_to_html(self, title_line, data, total_line):
        content = "<html><body>"
